...
const getArgValue = (arg: string): { name: string; value: string } => {
  ...
  if (equalIndex === -1) {
    throw new Error('引数の指定方法がおかしいでヤンス。「--component=atoms --name=ComponentName」にしてほしいでやんす');
  }
  ...
  if (name !== 'component' && name !== 'name') {
    throw new Error('引数が違うでヤンス。「--component=atoms --name=ComponentName」にしてほしいでやんす');
  }
...
const validateArgs = (args: string[]) => {
  if (args.length !== 5) {
    throw new Error('引数の数が正しくないでゲス。--componentと--nameを指定してほしいでゲス');
  }
};
...
const main = () => {
  try {
    ...
    writeFileSync(`${path}/index.tsx`, createComponentTemplate(componentName.value));
    writeFileSync(`${path}/styles.ts`, createStyleTemplate());
    writeFileSync(`${path}/index.stories.tsx`, createStorybookTemplate(pathName.value, componentName.value));
  } catch (error) {
